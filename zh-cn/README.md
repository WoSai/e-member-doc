...
本文档用于说明商户如何接入电子会员系统的会员数据。
 ...
 > * 第二步：准备好密钥secret，向收钱吧申请appid。注：为确保访问安全，秘钥有效期为一个月，商户应提供签到接口定期更新密钥(查看章节2.1)；
 > * 第三步：开发签到接口(查看章节2.1)；
 > * 第四步：开发接收会员信息推送的接口（查看章节3）；
 > * 第五步：开发查询会员信息的接口（查看章节4）；
...
###2.1 签到
    为了保证安全，商户应提供密钥给收钱吧，且应该定期更新，收钱吧通过调用商户的签到接口完成秘钥更新，新的秘钥的有效期默认为30天。
 - 接口地址：https://member.hm.com/api/sign/v2
 - 访问方式：post
 - 参数说明：
...
##3.会员卡信息推送
 ...
 - 接口地址：https://member.hm.com/api/member/notify/v1
 - 访问方式：post
 - 参数格式：json
 - 参数说明：
...
##4.根据会员卡标识查询会员卡信息
 ...
 - 接口地址：https://member.hm.com/api/member/query/v1
 - 访问方式：post
 - 参数格式：json
 - 参数说明：
